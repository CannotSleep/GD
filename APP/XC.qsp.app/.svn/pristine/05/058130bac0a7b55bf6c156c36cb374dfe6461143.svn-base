						...
						<view class="uni-triplex-row">
							<view class="uni-triplex-left">
								...
								<label>   
								    ...
								    <text class="list-level">{{item.Grade}}</text>
                                </label>
							</view>
						</view>
					</view>
	...
	export default {
		...
		methods: {
			...
			getListData(cid, pid) { //获取列表数据
				const url = server.net.setUrl('api/QualityConcreteBackApi/GetList?page=' + this.page);
				let data = {
				};
				if (cid) {
					data.CompanyId = cid;
				...
				uni.request({
					...
					success: res => {
						...
						if (jsondata.status == 1) {
							...
							this.ConcreteList = this.ConcreteList.concat(newItems);
						} else {
