...
namespace Tmp.Service
{
    ...
    public static class ServiceExtensions
    {
        ...
        public static DataSet GetListByPage<T>(this IService<T> service, PageParam pageParam, ref int total) where T : BaseEntity
        {
            ...
            DataSet ds = DbHelperSql.RunProcedure(DbHelperSql.DefaultQueryConn, "SP_COM_QUERY_BY_PAGE", parameters, "ds");//TO DO CREATE PROC
        ...
        /// <returns></returns>
        /// 2016-09-18 新增
        /// 2016-11-08 列的顺序以fieldList的为准
        public static DataTable ToDataTable<T>(this IService<T> service, IList list, List<GridColumn> fieldList) where T : BaseEntity
        {
            return ToDataTable(list, fieldList);
        }

        public static DataTable ToDataTable(IList list, List<GridColumn> fieldList)
        {
            DataTable result = new DataTable();
            if (list.Count > 0)
            {
