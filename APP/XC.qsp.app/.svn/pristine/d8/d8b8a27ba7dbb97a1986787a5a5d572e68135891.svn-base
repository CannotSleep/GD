		...
		methods: {
					...
					success: (res) => {
						...
						for(let f of fileArr){
							...
							for(let fchild of f.Detail){
								let value = uni.getStorageSync(fchild.ID);//改用id作为判断依据
								if(value){
									fchild.isDown = 1;
								}else{
									fchild.isDown = 0;
							...
							//保存文件
							 uni.saveFile({
									...
									success: function (result) {
									 ...
									 //保存后文件路径
									 //let tempfile = result.savedFilePath;
									 let tempfile = JSON.stringify(result.savedFilePath);
									 //原文件名
			...
			fileOpen(fileName){
			  ...
			  let newName =	uni.getStorageSync(fileName)
				...
				uni.openDocument({ 
					filePath:"_doc/uniapp_save/"+newName,
					success:function(result){
						console.log(JSON.stringify(result))
						console.log("打开成功");
					},
