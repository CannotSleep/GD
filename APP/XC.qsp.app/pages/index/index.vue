...
<script>
	...
	export default {
		onLoad() {
			...
			this.startGetPushMessage();
		},
		onUnload() {
			//程序退出后清空token 如果不自动登陆 下次需要登陆
		...
		methods: {
			...
			checkLogin() {
				...
				let sessionToken = server.net.GetSessionToken(); 
				console.log("localToken：" + localToken + ",sessionToken:" + sessionToken);
			...
			getClientID(){
				//#ifdef APP-PLUS
					...
					let url = server.net.setUrl('api/UserClientInfoApi/Bind');
					uni.request({
						url: url,
						method: 'POST',
						header: {
						...
						fail: (data, code) => {
							console.log("ClientID上传失败" + data + code);
						}
					});
				//#endif
			},
			startGetPushMessage(){
                ...
                plus.push.addEventListener( "receive", function( msg ) {
					if (msg.aps) {//ios应用在线会触发此事件,消息不进入通知栏
                        uni.navigateTo({
							url: '../message/messageList'
						});
					}
