	...
	<view class="content">
		<form action="" @submit="safetySubmit" @reset="formReset" class="input-form">
				...
				<view class="uni-list-cell cell-pd">
					<view class="uni-uploader">
						<view class="uni-uploader-head">
							<image class="imicon" src="../../../static/img/list1_icon/list1_icon-15.png" mode=""></image>
			...
			<view class="input-row" v-if="isRejected">
				...
				<input type="text" value="" v-model="rejuectContent" placeholder="请填写拒绝理由" />
			</view>
			<view class="input-row-bottom-line"></view>
...
<script>
	...
	export default {
		...
		methods: {
					...
					success: res => {
						...
						if (result.status == 1) {
							...
							jsondata.forEach((e) => {
								...
								this.SubcontractingUnitName = e.SubcontractingUnitName;
								...
								this.ExpirDate = e.ExpireDate.substring(0, 10);
								...
								this.ReState = e.State;
								this.RefuseReason = e.RefuseReason;
								switch (e.State) {
									case 0:
			...
			Review(state) {
				console.log(state);
				console.log(this.rejuectContent)
				let url = server.net.setUrl('api/SecurityInspectApi/Review');
				let ajaxdata = {
					id: this.pageId,
					state: state+'',
					content: this.rejuectContent
				}
